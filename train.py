...
# this script trains the ORNL Segmentation model using a combination of Cross Entropy Loss and Dice Loss
# Configuration
BATCH_SIZE = 4
LEARNING_RATE = 0.001
EPOCHS = 100
...
class DiceLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        targets_one_hot = F.one_hot(
            targets, num_classes=logits.shape[1]).float()
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            f"Epoch {epoch+1}: Train Loss = {train_loss_accum/len(train_loader):.4f} | Val Loss = {avg_loss:.4f}")
